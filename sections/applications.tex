...
%In variant calling and genotyping, reduced reference bias and complete representation of difficult variation in the reference can improve accuracy and 
%Here, we consider developments in several research areas.
...
%haplotype inference, functional genomics, and metagenomics.
%Pangenomic reference systems can improve the accuarcy of allele specific expression assays that are crucial to untangling diverse issues in functional genomics.
...
%They share information across genomes by considering collections of genomes at the same time in
...
%In principle, this can reduce the amount of novel sequence that must be considering during genome inference.
...
%, and it authors apply it to demonstrate that graph-aware genotyping results in higher accuracy at known variable sites 
...
%A sequence graph with labeled or colored paths provides a natural representation of a metagenomic or quasispecies pangenome.
...
%is the first tool that leverages information of short and long-reads to generate happlotype-resolved assembly. 

%generate mixed haplotype assembly and, therefore, fail to capture the diploid nature of the organism under study. 
...
%Genomic function manifests itself first in processes that lead to the expression of genomic information.
...
%Much of this work focuses on RNA sequencing (RNA-seq) analyses, but 
...
%\textsc{ASElux} takes a different approach and only aligns reads that overlaps heterozygous exonic SNVs \cite{Miao2018-ps}, but to similar effect as other variant-aware mappers.
...
When tested in 100 yeast strains, this approach marginally improves on standard GWAS techniques, but for some phenotypes provides a dramatic improvement in performance. %, presumably due to reference bias against alleles important to these phenotypes.

%connecting to the precision pangenomic models discussed in section \ref{sec:models}.


% I like this, but it's a little too far afield for this review -EG
