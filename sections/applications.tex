...
Incorporating known variation modestly improves Graphtyper's accuracy \cite{eggertsson2017graphtyper}.

Another set of variant callers are based on colored de Bruijn graphs.
Cortex pioneered this approach \cite{Iqbal_2012}.
It calls variants based on coverage in bubble structures in a colored de Bruijn graph, which is constructed from multiple read sets and reference genomes.
...
HLA*PRG \cite{dilthey2018hla} and its successor HLA*LA \cite{dilthey2019hla}, aligning reads to a graph of the HLA genes.
...
In constrast, Paragraph \cite{chen2019paragraph} and VG's genotyper are both based on aligning reads to graphs.
