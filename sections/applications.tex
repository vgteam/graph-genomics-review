...
In many ways, small variants stand to benefit the least from pangenomic variant calling.
...
However, pangenomes still have benefits to offer small variants, particularly around their use of the reference.
...
Both can cause variant calling errors.
Second, missing variation in the reference can cause base-level alignment artifacts.
These artifacts do not affect all variant calling algorithms equally, but in many tools they can be a further source of errors.

One strategy for genotyping small variants consists of using graphs of known variation in local regions to realign mapped reads.
This approach was pioneered in the 1000 Genomes Project by the GLIA software.
GLIA and FreeBayes were used to call genotypes on indels and complex variants, acknowledging the difficulties in aligning through such variants \cite{1000_2015}.
