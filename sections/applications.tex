...
Mykrobe predictor uses reference de Bruijn graphs to estimate antibody resistance from bacterial sequencing data \cite{Bradley2015-kl}.
...
The frequency of alleles and genes are predicted using read coverages using the intersection.
Using this approach the authors demonstrated accurate resistance prediction from \textit{S. aureus} and \textit{M. Tuberculosis} sequencing data within minutes on a laptop.  

GROOT \cite{Rowe2018-bg}

Virus-VG uses variation graphs to estimate virus haplotypes within a viral quasispecies.
Virus-VG first construct a variation graph reference from assembled contigs to which the sequencing reads are mapped using the vg toolkit .
Maximal-length candidate haplotype paths are then enumerated by merging overlapping contigs in a breadth-first search type procedure and their abundances estimated using an optimization algorithm that minimize the difference between predicted abundance and read coverage. 
...
This algorithm scales much better with genome size and using simulated and real data they show that it even improves accurate for the smaller viral genomes. 

One of the major metagenomics analysis pipelines involves the classification and abundance estimation of species and strains from the sequencing reads using a database of known sequences, such as bacterial and viral genomes. 
...
This method is based on the algorithm originally developed for RNA-seq analysis, Kallisto, and uses a colored de Bruijn graphs to represent a database of known genomes \cite{Bray_2016}. 
...
The authors demonstrates superior performance compared to state-of-the-art metagenomic abundance methods using simulated reads on a filtered set of 1027 bacterial genomes.
One area where graph based methods have been used extensively, is in metagenomic assembly, however, since this review mainly focuses on non-linear representations of reference structures we will not go into detail about them here. For more details on metagenomic assembly see Breitwieser \textit{et al.} review on assembly methods \cite{Breitwieser2017-yp}. 
