...
%Pangenomic models have many shapes, but in this review we will focus
...
These models represent the pangenome with a graph where nodes are labeled with DNA sequences and edges represent linkages between successive nodes that occur in some set of the genomes.
...
Assembly graphs represent the full information in a set of sequence reads \cite{Myers_2005}.
The \emph{de Bruijn graph} (dBG) \cite{Pevzner_2001}of the $k$-mers (sequences of fixed length $k$) of a collection of genomes is a specialized example of such a graph that is frequently applied to assembly and pangenomic applications.
...
%They greatly simplify the process of pangenome construction, which can be built from any kind of input data, and typically require only the specification of a length $k$ and an abundance threshold required to retain a particular $k$-mer.
...
%\todo{JAS: Could the dBG paragraph be simplified and shorten? It seems a bit too detailed for an introduction.}
...
%\todo{JAS: It took me couple of times to understand this sentence. Could the same thing be said without the language analogy?}
...
The genomes that a genome graph was built from can be found as walks through the graph.
But, because they lack information correlating these walks, simple genome graphs also include many sequences which are highly unlikely combinations of actual genomes.
To provide further structure to genome graphs, \emph{variation graphs} explicitly embed the observed linear sequences of the pangenome in the graph as \emph{paths}, or walks through the sequences of the graph \cite{Garrison_2018}.
