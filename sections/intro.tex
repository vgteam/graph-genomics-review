...
Our starting point is a sequence graph \cite{hein1989new} whose nodes are labeled\footnote{Such a graph can also be edge-labeled, with DNA sequences attached to its edges, but this is less common. For simplicity we will focus on the node-labeled model in this review.} with DNA sequences and whose edges represent linkages between successive nodes that are found in the set of sequences that the graph represents.
...
Paths provide a stable coordinate system that is unaffected by the manner in which the graph was built, and they make the model lossless and equivalent to the sequences from which it was built.
