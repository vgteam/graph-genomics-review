...
In the paper \citititle{Novak_2017a}, \citeauthor{Novak_2017a}\ compared different graph constructions based on their utility in read mapping, variant calling, and general graph character \citep{Novak_2017a} \todo{Does it really make sense to lean so heavily on our own unreviewed, unpublished manuscript in a review?}.
...
Some indexes \todo{Which?}\ support longer queries by doing extensive preprocessing.
In other indexes,  \todo{Which?}\  queries mapping to complex graph regions can be slow.
...
GCSA then applies a number of graph transformations that preserve the language, or set of admissable end-to-end sequences \todo{Is this correct?}, until the nodes can be unambiguously sorted by the labels of the paths starting from the node.
...
When the search encounters a variant, it must branch \todo{How must it branch? Or do we care?}\ to handle each allele separately.
While both BWBBLE and vBWT trade faster index construction for slower queries, a combination of the ideas can be quite practical \todo{What combination? Practical in what way?}\ \cite{Buechler_2019}.
