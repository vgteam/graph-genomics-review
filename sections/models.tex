...
\todo[inline]{SH: We did not introduce the concept of bubbles. Do we assume people know that?}
...
The recursive hierarchical alignment model in REVEAL \cite{linthorst2015scalable} finds a set of maximal exact unique matches of decreasing size between a pair of sequences, and uses these and the flanking un-matched sequences to build a sequence graph.
...
Its pangenome model is linear, but supports the post-hoc detection of inversions by aligning sequences in bubbles in the alignment graph.
...
NovoGraph follows a reference-guided approach, breaking a set of genomes into syntenic alignable blocks and deriving a multiple sequence alignment (MSA) for each, and yielding a VCF file as its output.
Similarly, seq-seq-pan employs existing whole genome alignment methods to find a set of locally co-linear blocks, in which it compacts into a sequence graph that respects the synteny of the its input genomes. GenGraph realigns previously identified co-linear blocks yielding a genome graph from an MSA.

Not all methods generate a pangenome model that relates all of their input sequences.
...
Althoug it does not embed the sequences (other than the first reference) which it was built from, the resulting graph should include representative versions of each genomic locus in the collection.
This may make it suitable as a technique to develop a single hierarchical coordinate system for a pangenome, or to extract a set of representative sequences from a collection of assembled genomes.
...
\todo{JAS: The above paragraph could maybe be shorten a bit? Seems to state the same twice.}
