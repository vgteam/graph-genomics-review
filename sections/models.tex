...
The \texttt{vg} tool offers multiple graph construction algorithms, including the de novo assembly algorithm MSGA (Multiple Sequence to Graph Aligner) \cite{Garrison_2018,Novak_2017a}. 
...
Seqwish, another de novo construction algorithm in the \texttt{vg} ecosystem, was developed to address the weaknesses of MSGA\footnote{\url{https://github.com/ekg/seqwish}}.
...
Starting with a graph composed entirely from a linear genome, MSGA constructs a more complex genome by using progressive calls to \texttt{vg} edit, which aligns and inserts the sequence to the graph. \todo{citation needed} \todo{Do we have a good publication detailing how Cactus graphs are constructed? Are there other \texttt{vg} algorithms I'm missing?}
...
Because \texttt{vg} provides an integrated toolkit for constructing, manipulating, and using genome graphs, it's a tool well-suited for comparing the efficacy of different genome graph construction techniques \cite{Novak_2017, Garrison_2018}. In the paper Genome Graphs, Novak et al. compared different graph constructions based on their utility in read mapping, variant calling, and general graph character.
