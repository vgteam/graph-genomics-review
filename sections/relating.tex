...
High-level assembly-graph tools like \textsc{Assembly Graph Browser} (\textsc{AGB}) \citep{Mikheenko_2019} (Figure~\ref{fig:visualization}D), and low-level variation graph tools like \textsc{vg view} \cite{Garrison_2018} and \textsc{Sequence Tube Map}, struggle with graphs that have both genomic scale and pangenomic detail.
...
Another particularly scalable tool, \textsc{odgi viz}\footnote{\url{https://github.com/vgteam/odgi}}, achieves its scalability by binning hundreds of adjacent nodes together.
This makes it possible to render the overall structure of gigabase pangenomes in a single image of manageable resolution.
It uses a very computationally efficient linearized layout technique developed for \textsc{vg viz} \citep{Garrison_2019} (Figure~\ref{fig:visualization}F), and shares that tool's distinctive visual language.
