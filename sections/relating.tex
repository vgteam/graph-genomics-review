...
Many tools, including the popular and venerable Bandage \citep{Wick_2015} (Figure~\ref{fig:visualization}A) and competitor GfaViz \cite{Gonnella_2018} (Figure~\ref{fig:visualization}B), are designed for interpreting assembly graphs. 
...
For example, Sequence Tube Map \cite{Beyer_2019} (Figure~\ref{fig:visualization}E) displays precise base-scale variation, haplotypes, and short read mapping locations, using a visual language inspired by transit system maps.
...
High-level assembly-graph tools like Assembly Graph Browser (AGB) \citep{Mikheenko_2019} (Figure~\ref{fig:visualization}D), and low-level variation graph tools like \texttt{vg view} \cite{Garrison_2018} and Sequence Tube Map, struggle with graphs that have both genomic scale and pangenomic detail.
...
Designed as a ``multi-scale'' graph browser, it presents both a Sequence Tube Map of base-level differences and a Circos \cite{Krzywinski_2009_Circos} plot of chromosomal-scale connections, and can uniquely visualize long reads in the context of pangenomic haplotypes \cite{yokoyama_momi-g:_2019}.
