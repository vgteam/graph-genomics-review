...
We present an overview of such tools in Table~\ref{table:Visualization_Features}, while Figure~\ref{fig:visualization} compares different visualization methods applied to the same graph. %relates different visualization methods applied to the same graph to demonstrate how different visualization approaches help to resolve different features of the graph.
...
%always understand the graph as being pangenomic and representing multiple individuals.
...
The approach is a rasterized version of the linear layout technique of \textsc{VG viz} \citep{Garrison_2019}. %, and shares that tool's distinctive visual language.
