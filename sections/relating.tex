...
It has now been shown that graph alignment can run at the same speed as non-graph alignment (in the computer scientist's sense of ``Big-O'' asymptotics).
...
Once a significant barrier to pangenomics research, the field now provides an array of well-performing, increasingly efficient read mapping tools for variation graphs. 
...
The field has not yet coalesced around a standard tool in the way conventional mapping now has around BWA-MEM\cite{Li_2013}. 

To the extent that a standard exists, it is currently VG\cite{Garrison_2018}---at least in the sense that later tools have largely chosen it as a point of comparison for performance\cite{Rautiainen_2019b,Kim_2019,Vaddadi_2019,Guo_2018}. 
It remains to be seen if VG will retain this position, or if another tool will emerge as a standard, or if a diversity of tools will be continue to be used. 
The ideal situation is probably be the latter. 
The available tools represent a menu of tradeoffs that researchers could match to their studies' unique requirements.

Invariably, variation graphs mappers make use of the seed-and-extend paradigm that has also been successful for conventional linear references.
This methodology consists of indexing the reference for exact matches ``seeds''.
Then, the index is queried for exact matches with a sequencing read, which are used to narrow in on small regions of the reference.
Expensive approximate-match alignment algorithms can then be targeted at these smaller regions to ``extend'' the seeds. 
As this description suggests, many of a variation graph mapping tool's features are determined by its choice of an indexing method and an alignment algorithm.
Variation graph indexing and alignment have both been active areas of research, as discussed above.
In addition, some tools employ an additional step to cluster seeds by their position on the graph before aligning.
While common for conventional mappers, this step has proven challenging in variation graphs, and some tools neglect it entirely.

\todo{JME: why are we going into so much detail about HISAT2 compred to the other tools?}
...
%	-oh, it seems this alignment step only applies to blocks, then tree unrolling
...
%-did not highlight alignment or mapping accuracy....
...
\todo{JME: This section does not have citations} 
\todo{JME: What about deBGA-VARA?} 
