...
For input data, graph visualization tools tend to target either genome assembly graphs or pangenome graphs use cases.
An overview of such tools is given in \ref{table:Visualization_Features}.
...
Layout paradigm also places restrictions on the scale of data a program can realistically process. SH: integrate this into the text flow.}
...
Sequence Tubemap renders regions of pangenome graphs using a visual language inspired by transit system maps \citep{Beyer_2019}. 
Its design approach is to keep the browser-based client but put more intelligence into the server.
This tool is useful for visualizing base-scale variation and short read mapping locations in human-chromosome-scale graphs.
...
MoMI-G is a multi-scale genome graph browser currently under development \cite{yokoyama_momi-g:_2019}. It employs SequenceTubemap at nucleotide scale and integrates it into a  framework along side a Circos plot for chromosome scale connections\cite{Krzywinski_2009_Circos}. 
The integration of multiple visualizations brings their strengths to bear on different areas. MoMI-G is currently the only tool that can visualize long reads to show structural variants against multiple haplotypes in a graph genome.

\todo[inline]{vg view force directed layout}
Other approaches to visualizing comprehensive pangenome graphs rely on restricting the problem in order to improve scalability.
...
The downside being it can be difficult to understand high-level or nonlinear structures in such a linearized layout. 
...
\todo[inline]{SH: This last sentence I would move to the discussion? JDS: I like it. I think its critical for concluding the section and transitioning.}
