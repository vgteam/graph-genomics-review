...
Existing pangenome graph visualization tools target different applications / audiences, dependent on the graph structure to visualize. 
...
we want people to think that graph visualization is important, not just a thing that exists. JDS: Hopefully, this is a more interesting start, then Simon and I will add our categorization intro. SH: I would delete the sentence before this comment or at least try to merge it with the previous one.}
...
\todo{JME: i love `noodles', but i don't know if it's a science word. SH: I don't think it is, but I also like it a lot :)}
...
The currently under heavy development multi-scale genome graph browser MoMI-G\cite{yokoyama_momi-g:_2019} takes the idea of SequenceTubeMap and integrates it into a multi-view visualization framework. 
MoMI-G's focus to display and browse nested SVs is realized in various viewing modules:  a) A chromosomal overview is realized with circos, b) the feature table view is for the filtering of SVs, c) the SequenceTubeMap shows the genome graph and d) a linear view of annotations complements the web applicaton. 
Though, as MoMI-G relies on SequenceTubeMap, is has the same disadvantages.
