...
Visualization of pangenome graphs presents novel challenges, because their data structure is convoluted, multidimensional, and nonlinear.
...
Examples include GenomeMapper (an early forerunner) \cite{Schneeberger_2009}, Seven Bridge's Graph Genome Aligner \cite{Rakocevic_2019}, HISAT2 \cite{Kim_2019}, V-MAP \cite{Vaddadi_2019}, and PaSGAL \cite{Jain_2019b}.
...
GraphAligner is notable in that it can apply to generic overlap graphs, including de Bruijn graphs, but its current graph indexing and alignment seeding strategy limits it to the alignment of long reads.
...
It uses a banded alignment algorithm to achieve impressive speed aligning long reads to genome graphs \cite{Rautiainen_2019b}, and requires around twice the runtime of minimap2 \cite{Li_2018} when the latter is configured to derive the full alignment (as GraphAligner does by default).
