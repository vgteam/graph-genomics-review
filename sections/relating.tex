...
\todo[inline]{JME: i think this section would benefit from a more inspirational lead-in. we want people to think that graph visualization is important, not just a thing that exists}

Bandage (\textbf{B}ioinformatics \textbf{A}pplication for \textbf{N}avigating \textit{\textbf{D}e novo} \textbf{A}ssembly \textbf{G}raphs \textbf{E}asily) \citep{Wick_2015} is one of the most popular \citep{Mikheenko_2019}.\todo{JME: IMO, spelling out these acronyms breaks up the flow and is not very important. bioinformatics acryonyms are shit, we don't need to elevate them in the text.}
...
\todo[inline]{JME: i'm worried some of these implementation details might be too in-the-weeds. they have important implications, but i think this discussion would be more accessible to a general audience if we refocus it on the effects that these implementation decisions have on the user }
...
\todo{JME: this lead-in is redundant with the introduction, but it could be rewritten to tie into that content. i'll do that later.}
...
\todo[inline]{JME: I think this section is missing a brief introduction to the seed and extend paradigm. that's probably where the discussion should start.}
...
\todo{JME: these are technical abbreviations that we can't assume the readers will know. really i think the commonality is that they're exact matches.}
...
\todo[inline]{JME: despite how it's introduced a lot of this content applies to non-k-mer seeds as well. some of this content could be reworked into a good introductory paragraph for this section}
...
% BlastGraph
    ...
    % Compute edit distances and alignments out in both directions from each seed
        % Deduplicate when multiple seed ssuggest the same alignment
