...
For data, graph visualization tools tend to target either genome assembly graphs or pangenome graphs use cases.
An overview of such tools is given in Table \ref{table:1}.

\begin{table}[h!]
\centering
\caption{Overview of pangenome visualization tools.}
\vspace{10mm}
\label{table:1}
\begin{tabular}{|p{2.5cm}|p{2cm}|p{2cm}|p{2cm}|p{2cm}|} 
 ...
 \hline
 \hline
Bandage & \multirow[c]{13}{=}{force-directed} & assembly graphs & \multirow{13}{=}{subregions of human-scale pangenome graphs, whole chromosomes possible, no nucleotide level} & interactive desktop application \\ \cline{1-1} \cline{3-3} \cline{5-5}
GfaViz &  & assembly graphs, variation graphs &  & interactive desktop application, GFA2 support, OGDF\cite{Chimani_2012_OGDF} \\ \cline{1-1} \cline{3-3} \cline{5-5}
SHIMMER Sketch Graph &  & k-mer graph &  & command line tool, static images, GraphViz and Gephi \\\hline
SGTK & force-directed, genome browser mode & scaffold graphs, assembly graphs, reads, reference sequences & megabyte scale, whole chromosomes, no nucleotide level & interactive web application, cytoscape.js \\\hline
AGB & rank-based layout, genome browser mode & assembly graphs, reference sequences & megabyte-scale, single human genome, no nucleotide level & interactive web application, d3-graphviz\footnote{\url{https://github.com/magjac/d3-graphviz}} \\\hline
SequenceTubeMap & pseudo-linear tubemap & \multirow{23}{=}{variation graph} & from nucleotides to kilobases, no whole chromosomes & interactive web application, unique layout \\ \cline{1-2} \cline{4-5}
MoMI-G & pseudo-linear tubemap, circos, linear annotation &  & gigabase circos, from nucleotides to kilobases, no whole chromosomes & interactive web application, multiple views \\ \cline{1-2} \cline{4-5}
vg view & force-directed &  & \multirow{7}{=}{nucleotide level, subgraph, no whole chromosomes} & command line tool, static image, GraphViz\\ \cline{1-2} \cline{5-5}
vg viz & rectangular sorted matrix &  &  & command line tool, static image \\ \cline{1-2} \cline{4-5}
odgi viz & rectangular sorted matrix &  & from nucleotide level to whole chromosomes of any size & command line tool, static image, binning \\\hline
\end{tabular}
\end{table}
...
\todo[inline]{JDS: Mention force directed graphs.  Mention Figure~\ref{fig:visualization} SH: In principal, all assembly graph viz tools order graphs with force-directed layout algorithms. There are OGDF, GraphViz, d3-graphviz, Gephi and Cytoscape.js. desktop apps: why is bandage so unique? why is gfa viz so unique? Cut down on details and focus on one or two features. -- Web apps SGTK + AGB -- what makes them special? Cut down text again. Where do we put SHIMMER?}
...
\todo[inline]{SH: now we present tools which were designed to visualize variation graphs}
