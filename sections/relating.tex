...
Either option is challenging, as pangenome graphs are often \textbf{nonplanar}, requiring at least three dimensions to draw without overlapping lines.
...
All 2D graph visualizations necessarily involve some degree of squashing, flattening, and compromising.  
...
Many tools, inclusing the popular and venerable Bandage \citep{Wick_2015} (Figure~\ref{fig:visualization}A), are designed for interpreting assembly graphs. 
...
To visualize assemblies in the relatively new GFA2 format, GfaViz \cite{Gonnella_2018} (Figure~\ref{fig:visualization}B) is a good Bandage alternative.

Tools initially designed for variation graph visualization, on the other hand, tend to focus more on base-level structure and the display of relationships between individual genomes and the pangenomic graph that contains them.
For variation graphs, Sequence Tube Map \cite{Beyer_2019} (Figure~\ref{fig:visualization}E) displays precise base-scale variation and short read mapping locations using a visual language inspired by transit system maps.
However, it can only usefully display a few kilobases at a time, and its interactivity suffers when the backing graphs grow past chromosome-scale.
...
High-level assembly-graph tools like Assembly Graph Browser (AGB) \citep{Mikheenko_2019} (Figure~\ref{fig:visualization}D), and low-level variation graph tools like \texttt{vg view} \cite{Garrison_2018}, are difficult to wield effectively when working with genome-scale, pangenome-detail graphs.
...
One tool which works well at both large scale and high detail is MoMI-G \cite{yokoyama_momi-g:_2019}. 
Designed as a ``multi-scale'' graph browser, it employs Sequence Tube Map at nucleotide scale, and integrates it alongside a Circos \cite{Krzywinski_2009_Circos} plot for chromosome-scale connections.
This allows it to present coherent views of both large-scale rearrangements and base-level differences.
MoMI-G is also currently the only tool that can visualize long reads to show structural variants against multiple haplotypes in a graph genome \cite{yokoyama_momi-g:_2019}.
...
Another particularly scalabel tool, \texttt{odgi viz}\footnote{\url{https://github.com/vgteam/odgi}}, achieves its scalability by binning hundreds of adjacent nodes together.
This makes it possible to render the overall structure of gigabase pangenomes in a single image of manageable resolution.
It uses a very computationally efficient linearized layout technique developed for \texttt{vg viz} \citep{Garrison_2019} (Figure~\ref{fig:visualization}F), and shares that tool's distinctive visual language.
...
%It remains an open problem to interactively visualize a comprehensive human pangenome graph, interchromosomal connections, at a variety of zoom levels to enable precision pangenomics at any scale.
